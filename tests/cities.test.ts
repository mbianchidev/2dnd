...
import { tryGridMove, useCityConnection } from "../src/systems/movement";
...
function findReachableTiles(
  ...
  while (queue.length > 0) {
    ...
    if (
      ...
      || !isWalkable(mapData[current.y][current.x])
    ) {
      continue;
    }
...
describe("multi-chunk city helpers", () => {
  ...
  describe("getCityChunkShopAt", () => {
    ...
    describe("getCityChunkShopNearby", () => {
      it("finds a shop through the active district carpet", () => {
        const city = makeMultiChunkCity();
        expect(getCityChunkShopNearby(city, 1, 4, 3)?.name)
          .toBe("Market District Shop");
      });
    });
...
describe("multi-chunk city movement", () => {
  ...
  it("uses extra chunk map data when cityChunkIndex > 0", () => {
    ...
    player.position.x = chunk.spawnX;
    player.position.y = chunk.spawnY;

    const result = tryGridMove(player, 1, 0);

    expect(result.moved).toBe(true);
    expect(player.position.x).toBe(chunk.spawnX + 1);
  });

  it("travels through city gates in both directions", () => {
    const player = createPlayer("GateWalker", defaultStats);
    const city = getCity("willowdale_city")!;
    player.position.inCity = true;
    player.position.cityId = city.id;
    player.position.cityChunkIndex = 0;
    ...
    expect(useCityConnection(player)).toBe(true);
    expect(player.position.cityChunkIndex).toBe(1);
...
describe("existing city data integrity", () => {
  ...
  it("all city chunks have a CityGate tile for district connections", () => {
    for (const city of CITIES) {
      for (let chunkIndex = 0; chunkIndex < getCityChunkCount(city); chunkIndex++) {
        const chunk = getCityChunk(city, chunkIndex)!;
        const hasGate = chunk.mapData.some((row) => row.includes(Terrain.CityGate));
        expect(
          hasGate,
          `${city.name} chunk '${chunk.name}' has no CityGate tile`,
        ).toBe(true);
      }
    }
  });

  it("city connections start on gates and end on walkable tiles", () => {
    for (const city of CITIES) {
      for (const connection of city.connections) {
        const source = getCityChunk(city, connection.fromChunkIndex)!;
        const destination = getCityChunk(city, connection.toChunkIndex)!;
        expect(source.mapData[connection.fromY][connection.fromX])
          .toBe(Terrain.CityGate);
        expect(isWalkable(destination.mapData[connection.toY][connection.toX]))
          .toBe(true);
        expect(getCityConnectionAt(
          city,
          connection.fromChunkIndex,
          connection.fromX,
          connection.fromY,
        )).toEqual(connection);
      }
  ...
  it("city connection gates are reachable from each district spawn", () => {
    for (const city of CITIES) {
      for (let chunkIndex = 0; chunkIndex < getCityChunkCount(city); chunkIndex++) {
        const chunk = getCityChunk(city, chunkIndex)!;
        ...
        const connections = city.connections.filter(
          (connection) => connection.fromChunkIndex === chunkIndex,
        );

        for (const connection of connections) {
          expect(
            reachable.has(`${connection.fromX},${connection.fromY}`),
            `${city.name} chunk '${chunk.name}' cannot reach gate at `
  ...
  it("uses unique terrain IDs for city and dungeon connectors", () => {
    const connectorIds = [
      Terrain.CityGate,
